...
// SPEC ﾂｧ5 is written against the legacy API. In SDK 57 the legacy functions
...
export default function DeviceProbe() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (perm.accessPrivileges === 'limited') {
        log('WARNING: limited access 窶・counts below are NOT the full library.');
      }
      ...
      // ---- 2b. pagination speed (ﾂｧ5.2 claims this is fast) ---------------
      log();
      ...
      // ---- 2c. screenshot detection fallbacks (ﾂｧ5.4) ---------------------
      log();
  ...
  return (
    <View style={styles.root}>
      ...
      <Text style={styles.title}>Latent 窶・device probe (SPEC ﾂｧ14)</Text>
      <Button
        title={running ? 'Running窶ｦ' : 'Run probe'}
        onPress={run}
        disabled={running}
      />
